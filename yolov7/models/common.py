...
class LDSCA(nn.Module):
    def __init__(self, in_channels, out_channels, k=1, s=1):
        ...
        self.depthwise1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1, groups=in_channels, bias=False)
        self.pointwise1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, groups=2, bias=False)
    ...
    def forward(self, x):
        ...
        outv = self.conv1(out)
        outv = self.softmax(outv)

        outu = self.conv2(out)

        outz = outv * outu
        outz = self.conv3(outz)
        out = out + outz
...
class DWSConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, act=True):
        ...
        self.conv1 = Conv(c1, c1, k, s, g=c1, act=act)
        self.conv2 = nn.Conv2d(c1, c2, 1, 1, 0, groups=2, bias=False)
