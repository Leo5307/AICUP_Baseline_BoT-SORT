...
for folder in "$SOURCE_DIR"/*; do
    timestamp=${folder##*/}
    python3 tools/mc_demo_bert_7_multi_camera.py --weights "$WEIGHTS" --source "$folder" --device "$DEVICE" --name "$timestamp" --fuse-score --agnostic-nms --with-reid --augment --fast-reid-config "$FAST_REID_CONFIG" --fast-reid-weights "$FAST_REID_WEIGHTS"
done
